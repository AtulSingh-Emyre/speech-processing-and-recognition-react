...
class Utils {
  ...
  ******************************************************************************/
  ...
  static mergeBuffers(channelBuffer) {
    if (channelBuffer.length === 0) return;
  ...
  static bufferToBlob(internalLeftChannel, internalRecordingLength) {
    ...
    var numChannels = 1,
        sampleRate = 48000; // we create our wav file

    var buffer = new ArrayBuffer(44 + interleaved.length * 2);
    var view = new DataView(buffer); // RIFF chunk descriptor

    this.writeUTFBytes(view, 0, 'RIFF'); // -8 (via #97)

    view.setUint32(4, 44 + interleaved.length * 2 - 8, true);
    this.writeUTFBytes(view, 8, 'WAVE'); // FMT sub-chunk
